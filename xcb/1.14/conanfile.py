...
class XcbConan(ConanFile):
    ...
    def source(self):
        tools.get(
            f"https://xcb.freedesktop.org/dist/libxcb-{self.version}.tar.gz"
        )

    def requirements(self):
        self.requires.add("xorg-util-macros/1.19.2", private=False)
        self.requires.add("xorgproto/2019.2", private=False)
        self.requires.add("xau/1.0.9", private=False)
        self.requires.add("xdmcp/1.1.3", private=False)
        self.requires.add("xcb-proto/1.14", private=False)
    ...
    def build(self):
        ...
        autotools.configure(
            ...
            , pkg_config_paths = [
                ...
                , f'''{self.deps_cpp_info["xcb-proto"].rootpath}/lib/pkgconfig'''
            ]
            , args = [
